        ...
        switch (waiting_call[i].level) {
            case 0: //先找符合要求的，再找下一个优先队列。
                if (!center.available_operator.empty()) {
                    ...
                    cout << "Operator " << person.serial << " process call " << " " << i + 1<< " call_level " << waiting_call[i].level 
                    ...
                    if (!center.available_supervisor.empty()) {
                        ...
                        cout << "Supervisor " << person.serial << " process call " << i + 1<< " call_level " << waiting_call[i].level 
                             << " end_time " << end_time << endl;
                        i++;
                    } else {
                        if (!center.available_director.empty()) {
                            ...
                            cout << "Director " << person.serial << " process call " << i + 1<< " call_level " << waiting_call[i].level 
                                 << " end_time " << end_time << endl;
                            i++;
                        } else {
